...
SELECT region FROM salespeople GROUP BY region ORDER BY region; 
...
SELECT orders.id, status, order_total FROM orders JOIN customers ON (orders.customer_id = customers.id) WHERE email = 'pclark74@gmail.com' ORDER BY  id; 